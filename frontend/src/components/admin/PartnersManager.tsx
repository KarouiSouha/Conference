...
export default function PartnersManager() {
  ...
  const handleSavePartner = async (formData: { id?: string; name_fr: string; name_en: string; image: File | string | null; type: string; removeImage?: boolean }) => {
    try {
      ...
      const url = isEditing
        ? `http://localhost:8000/api/Partners/update/${formData.id}`
      ...
      setPartners((prev) => {
        if (isEditing) {
          return prev.map((p) =>
            p.id === formData.id
              ? { ...p, nameFr: formData.name_fr, nameEn: formData.name_en, type: formData.type, image: result.partner.image }
              : p
          );
        }
        return [...prev, {
          ...
          image: result.partner.image || null,
          type: formData.type,
        }];
      });
      ...
      console.error(`Error ${formData.id ? 'updating' : 'creating'} partner:`, error);
    }
  };
  ...
  const handleOpenDeleteModal = (partner: Partner) => {
  ...
  const getImageUrl = (partner: Partner) => {
    ...
    return `http://localhost:8000/storage/${partner.image}?t=${Date.now()}`;
  };
